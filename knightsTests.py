...
#        genetic.Benchmark.run(lambda:self.test_10X10())
    ...
    @staticmethod
    def find_knight_positions(boardWidth,boardHeight,expectedKnights):
     ...
     #       return Position(random.randrange(0,boardWidth),
    ...
    while count>0:
        ...
        [p for positions in
         map(lambda x:get_attacks(x,boardWidth,boardHeight),unattacked)
          for p in positions if p in nonEdgePositions] if len(unattacked)>0 else nonEdgePositions
        geneIndex=random.randrange(0,len(genes)) if len(knightIndexes)==0\
...
class Board:
    ...
    def print(self):
        # 0,0 prints the bottom left corner
