...
def max_quantity(item,maxWeight,maxVolume):
    return min(int(maxWeight/item.Weight) if item.Weight>0 else sys.maxsize,int(maxVolume/item.Volume) if item.Volume>0 else sys.maxsize)
...
def display(candidate,startTime):
    ...
    print("{0}\t{1}\t{2}".format(', '.join(descriptions),candidate.Fitness,str(timeDiff)))
